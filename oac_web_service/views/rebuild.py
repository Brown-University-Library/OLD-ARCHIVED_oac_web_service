...
def rebuild():
    """
        A POST or GET method that will rebuild the internal TDB index.

        If GET, the entire TDB index will be rebuilt from Fedora objects

        If POST, include a pid paramter, and only that object in Fedora
        will be rebuilt in the TDB index.
    """
    if request.method == "POST":
        pid = request.form.get('pid', None)
        if pid is not None:
            return rebuild_one(pid)
    else:
        return rebuild_all()
